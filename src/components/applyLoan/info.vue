<template>
  <div class="info">
    ...
    <mt-field class="item" label="客户类型：" placeholder="客户类型" v-model="client_type" readonly @click.native="visible3 = true"></mt-field>
    <mt-field class="item" label="资金用途：" placeholder="资金用途" v-model="purpose" readonly @click.native="visible4 = true"></mt-field>
    ...
    <mt-popup v-model="visible0" class="popup_pic">
      <div class="pic">图片验证码</div>
      <div class="pic">
       <img :src="'data:image/jpg;base64,'+this.picture.imageBase64" alt="验证码图片" @click.native="changePicture">
          <span>点击图片更换</span>
      </div>
      <mt-field class="popupInput" label="验证码：" placeholder="请输入图片验证码" v-model="picture_code" :attr="{maxlength:4}"></mt-field>
          <mt-button plain style="width:80%;" type="primary" @click.native="dialogmethod">确定</mt-button>
    </mt-popup>
    <mt-popup v-model="visible1" position="bottom" class="popup">
      <div style="height:40px;">
        <span class="mint-datetime-action mint-datetime-cancel" @click="visible1 = false">取消</span>
...
<mt-popup v-model="visible2" position="bottom" class="popup">
      <div style="height:40px;">
...
<mt-popup v-model="visible3" position="bottom" class="popup">
      <div style="height:40px;">
        <span class="mint-datetime-action mint-datetime-cancel" @click="visible3 = false">取消</span>
  ...
  <mt-popup v-model="visible4" position="bottom" class="popup">
      <div style="height:40px;">
        <span class="mint-datetime-action mint-datetime-cancel" @click="visible4 = false">取消</span>
        <span class="mint-datetime-action mint-datetime-confirm" @click="confirm4">确定</span>
      </div>
      <mt-picker
        :slots="slots4"
        class="picker-up"
        ref="picker"
        :visibleItemCount="3"
        @change="pickerChange4">
      </mt-picker>
    </mt-popup>
   <mt-popup v-model="visible5" position="bottom" class="popup">
      <div style="height:40px;">
        <span class="mint-datetime-action mint-datetime-cancel" @click="visible5 = false">取消</span>
...
export default {
    ...
    methods: {
      pickerChange1(picker, values){
        this.tpmFee = values[0]
      },
      confirm1(){
        this.fee = this.tpmFee ? this.tpmFee : fee[1];
        this.visible1 = false
      },
      pickerChange2(picker, values){
        this.tpmtime_limit = values[0]
      },
      confirm2(){
        this.time_limit = this.tpmtime_limit?this.tpmtime_limit:time_limit[1]
        this.visible2 = false
      },
      pickerChange3(picker, values){
        this.tpmclient_type = values[0]
      },
      confirm3(){
        this.client_type = this.tpmclient_type?this.tpmclient_type:client_type[1]
        this.visible3 = false
      },
      pickerChange4(picker, values){
        this.tpmpurpose = values[0]
      },
      confirm4(){
        this.purpose = this.tpmpurpose?this.tpmpurpose:purpose[1]
        this.visible4 = false
      },
      pickerChange5(picker, values){
        if(values[0] !== this.tpmAddress[0]){
          allCity.forEach((item, index)=>{
            if(item.value == values[0]){
              this.slots5[2].values = allCity[index].children

              allArea.forEach((item, indexs)=>{
                if(item.value == cityData[0]){
                  this.slots5[4].values = allArea[indexs].children
                }
              })
            }
          })
        ...
        if(values[1] !== this.tpmAddress[1]){
          allArea.forEach((item, index)=>{
            if(item.value == values[1]){
              this.slots5[4].values = allArea[index].children
            }
          })
        }
      ...
      change() {
        if(this.form.client_type==='企业员工'||this.form.client_type==='企业主'){
          this.show1= true
          this.show2= false
        }
        if(this.form.client_type==='个体工商户'){
          this.show2= true
          this.show1= false
        }
      },
      changePicture() {
        this.$store.dispatch('picture_code', {
          appID: this.headInfo.appID,
          reqData: {
      ...
      sendMsgCode() {
        console.log(this.mobile.substring(0,1))
        if (!this.mobile || this.mobile.substring(0,1) != 1 || this.mobile.length !== 11) {
          return Toast({
            message: '请输入正确的手机号码',
            duration: 1500
          });
      ...
      dialogmethod(){
        if (this.picture_code.length!==4) {
          return this.$notify.error({
            message: '验证码位数不对',
          duration: 2000
          });
        }
        this.$store.dispatch('vali_picture_code', {
          appID: this.headInfo.appID,
          reqData: {
            sno: this.paramsObj.sno,
            original: 'huoke',
            mobile: this.form.mobile,
            validateCode: this.picture_code,
            processStep: '2'
          }
        }).then(res => {
          if (res.ok) {
      ...
      handleClick() {
        ...
        if (!this.form.mobile.startsWith('1') || this.form.mobile.length!==11) {
          return Toast({
            message: '请输入正确的手机号码',
            duration: 1500
          });
        }
        if (this.form.name.trim().length < 1||this.form.fee.length < 1||this.form.time_limit.length < 1||this.form.address_road.length < 1
                ||this.form.purpose.length < 1||this.sms_code.length < 3||this.form.address_area.length < 1||this.form.client_type.length < 1||this.picture_code.length!==4) {
          return Toast({
            message: '输入内容为空或有误',
            duration: 1500
          });
        }
        if ((this.show1 && this.form.company.length<1)||(this.show2 && this.form.industry.length<1)){
          return Toast({
            message: '输入内容为空或有误',
            duration: 1500
          });
        }
        this.Address= this.form.address_province+this.form.address_city+this.form.address_area+this.form.address_road
        let params = {
          appID: this.headInfo.appID,
          reqData: {
            sno: this.paramsObj.sno,
            ...
            barSize: 'S',
            name: this.form.name,
            clientIDCardNo: this.form.id,
            addressProvince: this.form.address_province,
            addressCity: this.form.address_city,
            addressArea: this.form.address_area,
            addressRoad: this.form.address_road,
            address: this.Address,
            company: this.form.company,
            industry: this.form.industry,
            clientType: this.form.client_type,
            purpose: this.form.purpose,
            timeLimit: this.form.time_limit,
            fee: this.form.fee,
            commerId: this.headInfo.commercrialId,
            custManager: this.headInfo.custManager,
            processStep: '2'
          }
        }
        this.$store.dispatch('cust_Loan_App',params)
          .then(res => {
            if (res.ok) {
              this.$emit('toNexT',{type: 'success'})
            } else{
              return Toast({
                message: res.msg,
                duration: 1500
    ...
    computed: {
      headInfo() {
        return this.$store.state.applyLoan.headInfo
      },
      paramsObj() {
        return this.$store.state.applyLoan.paramsObj
      },
      picture() {
        return this.$store.state.applyLoan.picture
      }
    }
}
</script>
...
.info .popupInput .mint-cell-value{
  margin-right: -20px;
}
.info .popup_pic{
  width:80%;
