<template>
  <div class="mer-qrcode">
    ...
    <mt-field class="item" label="经营类目：" placeholder="经营类目" v-model="item"></mt-field>
...
export default {
  ...
  methods: {
    ...
    pickChange(picker, values){
      ...
      if(values[2] !== this.tpmAddress[2]){
        
      }
      this.tpmAddress = values
      console.log(values)
    },
    handleClick() {
      ...
      if (this.form.name.trim().length < 1||this.form.address_area.length < 1||this.form.catag.length < 1||
        this.form.boss.trim().length < 1||this.form.address_road.trim().length < 1) {
        return Toast({
          message: '输入内容为空或有误',
          duration: 1500
        });
      ...
      let params = {
        ...
        reqData: {
          ...
          custManager: this.initialInfo.custManager,
        }
      }
      this.$store.dispatch('mer_Input', params)
        .then(res => {
          if (res.ok) {
            this.$emit('toNexT', {type: 'Mer'})
          } else {
            return Toast({
              message: res.msg,
              duration: 1500
